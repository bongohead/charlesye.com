...
/*** ac-assets-hm.js and ac-regions-hm.js are identical except for initialization & width/height multiplier (30 for assets, 20 for regions) ***/
				...
				setData(
					'userData',
					{
						...
						...{
							...
							acSeries: acSeries,
							playStateHm: 'pause',
							playIndexHm: acSeriesDates.length - 1
						}
					}
				);
				dfd.resolve();
	...
	$('#heatmap-container').on('click', '#heatmap-subtitle-group > button.heatmap-subtitle', function() {
		...
		if (ud.playStateHm == null) return;
           
		if (clickedPlayDirection === 'pause') {
			var newplayStateHm = 'pause';
			var newplayIndexHm = ud.playIndexHm;
		}
		else if (clickedPlayDirection === 'start' || clickedPlayDirection === 'end') {
			var newplayStateHm = 'pause';
			var newplayIndexHm = (clickedPlayDirection === 'start') ? 0 : ud.acSeriesDates.length - 1;
		}
		/* If click back & index greater than 5, head back by 5, otherwise 0 */
			...
			var newplayStateHm = clickedPlayDirection;
			if (clickedPlayDirection === 'back') var newplayIndexHm = (ud.playIndexHm >= 5) ? ud.playIndexHm - 5 : 0;
			else var newplayIndexHm = (ud.playIndexHm + 5 <= ud.acSeriesDates.length - 1) ? ud.playIndexHm + 5 : ud.acSeriesDates.length;
		}
		console.log('clicked', clickedPlayDirection, newplayStateHm, newplayIndexHm);
				           
		setData('userData', {...getData('userData'), ...{playStateHm: newplayStateHm, playIndexHm: newplayIndexHm}});
		if (clickedPlayDirection !== 'pause') updateHeatmap();
		else updateButtonsHm(newplayStateHm, newplayIndexHm, ud.acSeriesDates);
...
function drawHeatmap(acFund, acFundSeriesMap, acSeries, acActiveDate) {
	...
	const chart = Highcharts.chart('heatmap-container', {
		chart: {
			height:350,
			//height: Math.floor(acFund.length * 10),
			//width: Math.floor(acFund.length * 30) + 80,
			/*marginTop: 110,
			marginRight: 100,
			marginBottom: 200,
			marginLeft: 200,*/
		...
		subtitle: {
			...
			'<div class="col-auto">'+
				'<div class="alert alert-primary my-0 py-1" style="max-width:360px"><span style="font-size:1.0rem">Data as of&nbsp;<span class="d-inline" id="heatmap-subtitle-date">'+ moment(acActiveDate).format('MMM D YY') +'</span></span></div>'+
			'</div>'+
			'<div class="col-auto"><div class="btn-group d-inline-block" role="group" id="heatmap-subtitle-group">' +
				'<button class="btn btn-secondary btn-sm" style="font-size:.8rem" type="button" disabled>Click to play&nbsp;</button>'+
		...
		colorAxis: {
			...
			stops:[
				...
				[0.4, 'rgba(0,132,255,1)'], //lightblue
				[0.5, 'rgba(0,212,255,1)'], //cyan
				[0.6, 'rgba(179,255,179,1)'], //green
				[0.7, 'rgba(253,255,53,1)'], //yellow
				[0.85, 'rgba(255,160,0,1)'], //orange
...
function updateHeatmap() {
	...
	/* Return if playStateHm = paused; this is necessary to shut off the auto-repeating nature of this function */
	if (ud.acSeriesDates === undefined || ud.playIndexHm === undefined) return;
	const timeStart = new Date().getTime();
  
	/* Get the new active date */
    const acActiveDate = ud.acSeriesDates[ud.playIndexHm];
	...
	getAcSeriesDfd.done(function(acSeries) {
		...
		if (ud.playIndexHm <= 0 ) {
		  ud.playStateHm = 'pause';
		  ud.playIndexHm = 0;
		}
		else if (ud.playIndexHm >= ud.acSeriesDates.length - 1) {
		  ud.playStateHm = 'pause';
		  ud.playIndexHm = ud.acSeriesDates.length - 1;
		}
		else {
		  if (ud.playStateHm === 'forward')  ud.playIndexHm = ud.playIndexHm + 5; //skip by week
		  else ud.playIndexHm = ud.playIndexHm - 5; //skip by week
		}
		
		updateButtonsHm(ud.playStateHm, ud.playIndexHm, ud.acSeriesDates);
		...
		if (ud.playStateHm !== 'pause') setTimeout(function() {updateHeatmap();}, timeWait);
		return;
	});
	return;
}
...
function updateButtonsHm(playStateHm, playIndexHm, acSeriesDates) {
	
	/* Update buttons */
	const buttons = $('#heatmap-subtitle-group').find('button.heatmap-subtitle').removeClass('active').prop('disabled', false).end();
	console.log('updateButtonsHm', playStateHm, playIndexHm, acSeriesDates.length - 1, buttons);

	if (playIndexHm === 0) buttons.find('[data-dir="start"],[data-dir="back"]').prop('disabled', true);
	else if (playIndexHm === acSeriesDates.length - 1) buttons.find('[data-dir="end"],[data-dir="forward"]').prop('disabled', true);
	else if (playStateHm === 'pause') buttons.find('[data-dir="pause"]').addClass('active', true);
	else if (playStateHm === 'back') buttons.find('[data-dir="back"]').addClass('active', true);
	else if (playStateHm === 'forward') buttons.find('[data-dir="forward"]').addClass('active', true);
